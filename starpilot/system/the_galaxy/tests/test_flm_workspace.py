...
def _write_custom_trial_report(module, report_id="report-custom"):
  ...
  recommended_value = min(float(supported[first_symbol]["max"]), float(stock_knobs[first_symbol]) + 0.05)
  profile = {
    ...
    "genericParams": {"AdvancedLateralTune": True, "SteerLatAccel": 2.0},
    "flmOverrides": {
      "schemaVersion": 1,
      "baseFrictionThresholds": {},
...
def test_uploaded_rlogs_are_saved_listed_and_resolved(tmp_path):
  ...
  result = module.save_uploaded_rlogs(uploaded, "Motorway test")
  route_name = result["upload"]["routeName"]
  listed = result["workspace"]["uploads"]
  sources, warnings = module.resolve_route_sources([route_name], [str(tmp_path)])
  ...
  module.delete_uploaded_rlogs(result["upload"]["uploadId"])
...
def test_uploaded_rlogs_reject_wrong_compression_and_oversized_files(tmp_path, monkeypatch):
  ...
  with pytest.raises(ValueError, match="compression format"):
    module.save_uploaded_rlogs([SimpleNamespace(filename="rlog.zst", stream=BytesIO(b"not-zstd"))])
  assert module.list_uploaded_rlogs() == []

  monkeypatch.setattr(module, "FLM_UPLOAD_MAX_FILE_BYTES", 4)
  with pytest.raises(ValueError, match="too large"):
    module.save_uploaded_rlogs([SimpleNamespace(filename="rlog", stream=BytesIO(b"12345"))])
  assert module.list_uploaded_rlogs() == []
...
def test_custom_trial_schema_exposes_every_supported_knob_and_analysis_seed(tmp_path):
  ...
  assert recommended["genericParams"]["SteerLatAccel"] == pytest.approx(2.0)
