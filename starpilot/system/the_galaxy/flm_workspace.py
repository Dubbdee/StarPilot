...
def _add_parameters_start_here(capabilities: dict[str, Any], suggestions: list[dict[str, Any]], primary_path_key: str) -> list[str]:
  ...
  if not capabilities.get("richProfileKey"):
    lines.append("This car does not expose richer vehicle-specific FLM knobs yet, so generic advanced params and friction-threshold trials are the first code-level moves to test.")
  return lines
